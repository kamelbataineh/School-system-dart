...
//   final int classId;
//   final Map<String, int> marks; // Optional
...
class Student {
  ...
  bool login() {
    while (true) {
      ...
      if (_users.containsKey(inputUsername) && _users[inputUsername] == inputPassword) {
        print("Login successful!\n");
        return true;
      } else {
        print("Invalid username or password. Please try again.");
  ...
  void enterMarks() {
    while (true) {
      stdout.write("Enter mark for Arabic: ");
      int markArabc = int.parse(stdin.readLineSync()!);

      marks['Arabic'] = markArabc;
      ...
      marks['Science'] = markScience;

      print("Marks entered successfully for $username.");
      stdout.write("Do you want to enter marks for another student? (yes/no): ");
      String response = stdin.readLineSync()!.toLowerCase();
      if (response != 'yes') {
        break;
      }
